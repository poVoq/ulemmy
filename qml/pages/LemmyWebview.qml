	...
	Connections {
		...
		onShareRequested: {
			if ( transfer.contentType === ContentType.Text ) {
				...
				webView.url = (appSettings.instance.indexOf("http") != -1 ? appSettings.instance : "https://" + appSettings.instance) + "/create_post?body=" + message
			}
				...
				webView.url = (appSettings.instance.indexOf("http") != -1 ? appSettings.instance : "https://" + appSettings.instance) + "/create_post?url=" + message
			}
		}
	}
				...
				leadingActionBar {
					...
					actions:[									
						Action {
							...
							onTriggered: {
								webviewPage.currentView().url = helperFunctions.getInstanceURL() +"/communities"
						...
						Action {
							...
							iconName:"edit"
							onTriggered: {
								webviewPage.currentView().url = helperFunctions.getInstanceURL() +"/create_post"
								instancBottomEdge.collapse()
							}
						},
						Action {
							...
							onTriggered: {
								webviewPage.currentView().url = helperFunctions.getInstanceURL() +"/inbox"
								instancBottomEdge.collapse()
							}
						},
						Action {
							...
							onTriggered: {
								webviewPage.currentView().url = helperFunctions.getInstanceURL() +"/"
								instancBottomEdge.collapse()
							}
						}
					]
